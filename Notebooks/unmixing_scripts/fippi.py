...
class FIPPIExtractor(EndmemberExtractor):
	def __init__(self, profile):
		self.profile = profile
		self.profile_filename = "fippi.prof"
		super(FIPPIExtractor, self).__init__()
	...
	def extract_endmembers(self, hsi_3d, n_endmembers):
		...
		endmembers, indicies = profile_code(self.profile,
				self.profile_filename)(FIPPI)(hsi_2d, n_endmembers)
		return endmembers

def main(in_filename, out_filename, n_components, profile):
	extractor = FIPPIExtractor(profile)
